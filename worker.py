  ...
  """
        ...
        if thumbs in ('list-orphans', 'delete-orphans'):
            ...
            for root, dirs, files in os.walk(
                os.path.join(settings.MEDIA_ROOT, 'thumbs')
            ):
                for file in files:
                    if file[0] != '.':
                        ths[media.get_thumb_info(file, append_suffix=False)['rel']] = (
                            True
                        )
        ...
        try:
            last1 = Entry.objects.filter(service__public=True).order_by(
                '-date_published'
            )[0]
        except IndexError:
            last1 = None
        ...
        try:
            last2 = Entry.objects.filter(service__public=True).order_by(
                '-date_published'
            )[0]
        except IndexError:
            last2 = None
