...
class Service(models.Model):
    ...
    cls = models.CharField(_('Custom CSS Class'), max_length=16, null=True, blank=True)
    url = models.CharField(_('URL'), max_length=255, blank=True)
    user_id = models.CharField(
        _('User ID'), max_length=64, blank=True, help_text=_('Optional User ID')
    )
    creds = models.CharField(_('Credentials'), max_length=128, null=True, blank=True)
    ...
    home = models.BooleanField(
        ...
        default=True,
        help_text=_(
            'If unchecked, this stream will be still active, but hidden and thus '
...
class Entry(models.Model):
    ...
    date_published = models.DateTimeField(
        _('Date published'), null=False, blank=True, default=now, db_index=True
    )
    date_updated = models.DateTimeField(
        _('Date updated'), null=False, blank=True, default=now
    )
    date_inserted = models.DateTimeField(
        'Date inserted', null=False, blank=True, default=now, editable=False
    )
    guid = models.CharField('GUID', max_length=200)
    author_name = models.CharField(_("Author's Name"), max_length=64, blank=True)
    author_email = models.EmailField(_("Author's Email"), blank=True)
    author_uri = models.CharField(_("Author's URI"), max_length=128, blank=True)
    geolat = models.DecimalField(
        _('Geo Latitude'), max_digits=13, decimal_places=10, blank=True, null=True
    )
    geolng = models.DecimalField(
        _('Geo Longitude'), max_digits=13, decimal_places=10, blank=True, null=True
    )
    idata = models.CharField('Internal data', max_length=64, blank=True, editable=False)
    ...
    draft = models.BooleanField(
        ...
        default=False,
        help_text=_(
            'A draft is a post that is in progress. Only you will be able to see it.'
        ),
    )
    ...
    reblog = models.BooleanField(
        _('Reblog'), default=False, help_text=_('Reblogged post.')
    )
...
class List(models.Model):
    ...
    slug = models.CharField(
        _('Slug'), max_length=48, null=False, blank=True, editable=False
    )
